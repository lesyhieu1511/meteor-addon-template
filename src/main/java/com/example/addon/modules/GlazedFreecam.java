...
public class GlazedFreecam extends Module {
    ...
    public GlazedFreecam() {
        super(AddonTemplate.CATEGORY, "glazed-freecam", "Move freely.");
    }
    ...
    @EventHandler private void onRespawn(PacketEvent.Receive event) {
        if (event.packet instanceof ClientboundRespawnPacket) { restoreView(); toggle(); }
    }
    ...
    public void onGameRender() {
        ...
        double moveSpeed = speed.get() * 2.0 * (isBoundDown(mc.options.keySprint) ? 2.0 : 1.0);
        double moveX = 0, moveY = 0, moveZ = 0;
        if (isMovingForward) { moveX += forward.x * moveSpeed; moveZ += forward.z * moveSpeed; }
        if (isMovingBackward) { moveX -= forward.x * moveSpeed; moveZ -= forward.z * moveSpeed; }
        if (isMovingRight) { moveX += right.x * moveSpeed; moveZ += right.z * moveSpeed; }
        if (isMovingLeft) { moveX -= right.x * moveSpeed; moveZ -= right.z * moveSpeed; }
    ...
    @EventHandler(priority = EventPriority.HIGH) private void onMouseScroll(MouseScrollEvent event) {
    ...
    public void updateRotation(double deltaYaw, double deltaPitch) {
        yaw = Mth.wrapDegrees(yaw + (float) deltaYaw);
        pitch = Mth.clamp(pitch + (float) deltaPitch, -90.0f, 90.0f);
    }

    public double getInterpolatedX(float partialTicks) { return currentPosition.x; }
    public double getInterpolatedY(float partialTicks) { return currentPosition.y; }
    public double getInterpolatedZ(float partialTicks) { return currentPosition.z; }
    public double getInterpolatedYaw(float partialTicks) { return yaw; }
    public double getInterpolatedPitch(float partialTicks) { return pitch; }

    private void resetMovementKeys() {
        if (mc.options == null) return;
        mc.options.keyUp.setDown(false); mc.options.keyDown.setDown(false);
        mc.options.keyRight.setDown(false); mc.options.keyLeft.setDown(false);
